...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(HeartCode_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    HeartCode_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(HeartCode_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(HeartCode_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(HeartCode_M, 0);

  /* Initialize model */
  HeartCode_initialize();
