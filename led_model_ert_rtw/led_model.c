...
void led_model_step(void)
{
  ...
  /* Constant: '<S1>/Hysterisis (ms)' */
  led_model_B.Hysterisisms = led_model_P.Hysterisisms_Value;

  /* Constant: '<S1>/PVARP (ms)' */
  led_model_B.PVARPms = led_model_P.PVARPms_Value;

  /* Constant: '<S1>/Mode' */
  led_model_B.Mode = led_model_P.Mode_Value;

  /* MATLABSystem: '<S1>/ATR_CMP_DETECT (Boolean)' */
  if (led_model_DW.obj.SampleTime != led_model_P.ATR_CMP_DETECTBoolean_SampleTim)
  {
    led_model_DW.obj.SampleTime = led_model_P.ATR_CMP_DETECTBoolean_SampleTim;
  }

  led_model_B.ATR_CMP_DETECTBoolean = MW_digitalIO_read
    (led_model_DW.obj.MW_DIGITALIO_HANDLE);

  /* End of MATLABSystem: '<S1>/ATR_CMP_DETECT (Boolean)' */

  /* MATLABSystem: '<S1>/VENT_CMP_DETECT (Boolean)' */
  if (led_model_DW.obj_g.SampleTime !=
      led_model_P.VENT_CMP_DETECTBoolean_SampleTi) {
    led_model_DW.obj_g.SampleTime = led_model_P.VENT_CMP_DETECTBoolean_SampleTi;
  }

  led_model_B.VENT_CMP_DETECTBoolean = MW_digitalIO_read
    (led_model_DW.obj_g.MW_DIGITALIO_HANDLE);

  /* End of MATLABSystem: '<S1>/VENT_CMP_DETECT (Boolean)' */
  ...
  if (led_model_DW.is_active_c3_led_model == 0U) {
    ...
    switch (led_model_DW.is_c3_led_model) {
     ...
     case led_model_IN_AOO_Charging:
      ...
      if (led_model_DW.temporalCounter_i1 >= (uint32_T)ceil(60000.0 /
        ...
        if ((led_model_DW.temporalCounter_i1 >= (uint32_T)ceil
             (led_model_B.A_RefractoryPeriodms)) && ((led_model_B.Mode == 3.0) &&
     ...
     case led_model_IN_Main:
      ...
      led_model_B.GREEN_LED = false;
      led_model_B.FRONTEND_CTRL = true;
      if ((led_model_B.Mode == 2.0) || (led_model_B.Mode == 3.0)) {
        led_model_DW.is_c3_led_model = led_model_IN_AOO_Charging;
        led_model_DW.temporalCounter_i1 = 0U;
        led_m_enter_atomic_AOO_Charging();
      } else {
        if ((led_model_B.Mode == 0.0) || (led_model_B.Mode == 1.0)) {
     ...
     case led_model_IN_VOO_Charging:
      ...
      if (led_model_DW.temporalCounter_i1 >= (uint32_T)ceil(60000.0 /
        ...
        if ((led_model_DW.temporalCounter_i1 >= (uint32_T)ceil
             (led_model_B.V_RefractoryPeriodms)) && ((led_model_B.Mode == 1.0) &&
...
void led_model_initialize(void)
{
  ...
  /* External mode info */
  led_model_M->Sizes.checksums[0] = (2886376191U);
  led_model_M->Sizes.checksums[1] = (1933565180U);
  led_model_M->Sizes.checksums[2] = (152612308U);
  led_model_M->Sizes.checksums[3] = (612289369U);
...

  {
    ...
    /* Start for Constant: '<S1>/Hysterisis (ms)' */
    led_model_B.Hysterisisms = led_model_P.Hysterisisms_Value;
    ...
    /* Start for MATLABSystem: '<S1>/ATR_CMP_DETECT (Boolean)' */
    ...
    /* Start for MATLABSystem: '<S1>/VENT_CMP_DETECT (Boolean)' */
...
{
  /* Terminate for MATLABSystem: '<S1>/ATR_CMP_DETECT (Boolean)' */
  matlabCodegenHandle_matlabCodeg(&led_model_DW.obj);

  /* Terminate for MATLABSystem: '<S1>/VENT_CMP_DETECT (Boolean)' */
